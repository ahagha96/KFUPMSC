package agha.kfupmscapp.Activities.AllTeamsActivity;
...
import agha.kfupmscapp.Activities.AllTeamsActivity.TeamsApiClasses.AllTeamsApiCalls;
import agha.kfupmscapp.Activities.AllTeamsActivity.TeamsApiClasses.DisplayObject;
import agha.kfupmscapp.Activities.AllTeamsActivity.TeamsApiClasses.TeamObject;
...
public class AllTeamsActivity extends AppCompatActivity {
    ...
    @BindView(R.id.all_teams_search_bar)
    EditText searchBar;

    @BindView(R.id.all_teams_grid_view)
    GridView teamsGridView;

    @BindView(R.id.all_teams_swipe)
    ...
    private ArrayList<DisplayObject> teamsArrayList;
    ...
    private ArrayList<DisplayObject> resultArrayList;
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                switch (id){
                    ...
                    case R.id.nav_menu_main:
                        intent = new Intent(AllTeamsActivity.this, MainActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_teams:
                        //intent = new Intent(AllTeamsActivity.this, AllTeamsActivity.class);
                        //startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_players:
                        intent = new Intent(AllTeamsActivity.this, AllPlayersActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_matches:
                        intent = new Intent(AllTeamsActivity.this, MatchesActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_news:
                        intent = new Intent(AllTeamsActivity.this, AllNewsActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_scorers:
                        intent = new Intent(AllTeamsActivity.this, ScorerActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_champions:
                        intent = new Intent(AllTeamsActivity.this, ChampoinsActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_groups:
                        intent = new Intent(AllTeamsActivity.this, GroupsActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_about:
                        intent = new Intent(AllTeamsActivity.this, AboutClubActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;

                    case R.id.nav_menu_dev:
                        intent = new Intent(AllTeamsActivity.this, DevTeamActivity.class);
                        startActivity(intent);
                        drawerLayout.closeDrawer(GravityCompat.END);
                        break;
                }
    ...
    private void defineVariables() {
        inflater = LayoutInflater.from(AllTeamsActivity.this);
        teamsArrayList = new ArrayList<>();
        resultArrayList = new ArrayList<>();
        gridViewAdapter = new GridViewAdapter(teamsArrayList);
    ...
    private class GridViewAdapter extends BaseAdapter{
        ...
        TextView teamName;
        ImageView teamImage;
        ArrayList<DisplayObject> arrayList ;

        public GridViewAdapter(ArrayList<DisplayObject> arrayList){
        ...
        public View getView(final int i, View view, ViewGroup viewGroup) {
            // inflate layout
            view = inflater.inflate(R.layout.layout_rv_teams,null);
            // define views
            teamImage = (ImageView) view.findViewById(R.id.main_layout_rv_team_image);
            teamName = (TextView) view.findViewById(R.id.main_layout_rv_team_name);
            // bind
            teamName.setText(arrayList.get(i).getName());
            Ion.with(AllTeamsActivity.this)
                    .load(arrayList.get(i).getLogo())
                    .withBitmap()
                    .intoImageView(teamImage);

            if (reachedEndOfList(i) && isFinish == false && isSearchOn == false){
                new GetAllTeams().execute(nextPage);
            }

            view.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(AllTeamsActivity.this, DisplayTeam.class);
                    intent.putExtra("teamID",arrayList.get(i).getId());
                    startActivity(intent);
                }
            });
    ...
    // get all teams async
    private class GetAllTeams extends AsyncTask<Integer,Void,Void>{

        private AllTeamsApiCalls apiInterface;

        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            // define interface
            apiInterface = ApiClient.getApiClient().create(AllTeamsApiCalls.class);
        }

        @Override
        protected Void doInBackground(Integer... integers) {

            Call<TeamObject> call = apiInterface.getTeams(integers[0]);
            call.enqueue(new Callback<TeamObject>() {
                @Override
                public void onResponse(Call<TeamObject> call, Response<TeamObject> response) {
                    ...
                    for (int index = 0 ; index<response.body().getData().size() ; index++){
                        teamsArrayList.add(new DisplayObject(response.body().getData().get(index).getName(),
                                response.body().getData().get(index).getLogo(),response.body().getData().get(index).getId()));
                    }
                ...
                @Override
                public void onFailure(Call<TeamObject> call, Throwable t) {
    ...
    private class GetSearchResults extends AsyncTask<String, Void, Void>{
        ...
        protected Void doInBackground(String... strings) {
            Call<TeamObject> call = apiInterface.searchTeams(strings[0]);
            call.enqueue(new Callback<TeamObject>() {
                @Override
                public void onResponse(Call<TeamObject> call, Response<TeamObject> response) {
                    resultArrayList.clear();
                    for (int index = 0 ; index<response.body().getData().size() ; index++){
                        resultArrayList.add(new DisplayObject(response.body().getData().get(index).getName(),
                                response.body().getData().get(index).getLogo(),response.body().getData().get(index).getId()));
                    }
                    // update adapter
